...
- 商品一致性判断**必须只基于图片对比**（视频帧商品 vs 挂车商品全量图片）
- 如果 `product_category` 与挂车商品图片显示的商品不一致，**以挂车商品图片为准**
- 禁止因为 `product_category` 元数据与图片不一致而判定为 IPP（见 FP-10）
- **商品未物理出镜豁免（FP-11）**：有讲品但商品未以实物形式物理展示（仅屏幕/截图/口头），不算 IPP

### 重要：全量商品图片对比

- `Product_image` 字段为 **JSON 数组字符串格式**，例如：`["url1", "url2", "url3"]`
- 包含该商品的**所有图片**（主图、细节图、不同颜色/款式图等）
- **商品一致性判断规则**：只要视频帧中的商品与**任意一张**商品图片在视觉上一致 → 判定为一致（no_hit）
- 只有当视频帧中的商品与**所有**商品图片都不一致时 → 才进入豁免检查
- 不再仅对比单张主图，大幅降低因只对比主图导致的误伤
...
4. **商品未物理出镜（FP-11）**：有讲品但商品未以实物形式物理展示（仅屏幕/截图/口头）→ 豁免（FP-11）
...
Step 2: 商品对比（视频帧商品 vs 挂车商品全量图片，任意一张一致即算一致）
    ├─ 首先检查：视频帧中是否有挂车商品的物理实物展示？
    │   ├─ 无物理展示（仅屏幕/截图/口头）→ 豁免（FP-11）
    │   └─ 有物理展示 → 继续对比
    ├─ 一致 → 不命中
    └─ 不一致 → 进入豁免检查
